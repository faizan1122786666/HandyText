...
const OPTIONS = [
  {
    ...
    topBar: 'bg-[#3461ff] group-hover:bg-[#2b51d6]',
    badge: 'OCR Scanner',
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {OPTIONS.map((opt) => {
          ...
          return (
            ...
            >
              <div className={`h-1.5 w-full transition-colors duration-300 ${opt.topBar}`} />
              <div className="flex h-full flex-col p-5 sm:p-6">
                <div className="mb-5 flex items-start justify-between gap-3">
                  <div className={`flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl border border-white shadow-sm ring-1 ring-slate-200/70 transition-all duration-300 group-hover:scale-105 group-hover:bg-[#3461ff] group-hover:text-white group-hover:ring-[#3461ff]/20 sm:h-14 sm:w-14 ${opt.iconBg} ${opt.iconColor}`}>
                    <Icon size={24} />
                  </div>
                ...
                <div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-4">
                  <span className={`flex w-full items-center justify-between rounded-2xl px-5 py-3.5 text-white shadow-md transition-all duration-300 group-hover:-translate-y-0.5 group-hover:shadow-xl ${opt.buttonClass}`}>
                    ...
                    <span className="flex h-10 w-10 items-center justify-center rounded-full bg-white text-[#3461ff] shadow-sm transition-all duration-300 group-hover:translate-x-1 group-hover:scale-105 group-hover:bg-[#2b51d6] group-hover:text-white">
